...
export async function bench(): Promise<Benchmark.Suite> {
    const ped = generatePedersenParams(tomEdwards256),
        ...
        json = writeJson(EqualityProof, proof)
    console.log('proof size EqualityProof:', json.length)

    return new Benchmark.Suite()
        .add(`Equality/prove`, {
            defer: true,
            async fn(deferred: { resolve(): void }) {
                await proveEquality(ped, x, C1, C2)
                deferred.resolve()
            },
        })
        .add(`Equality/verify`, {
            defer: true,
            async fn(deferred: { resolve(): void }) {
                await verifyEquality(ped, C1.p, C2.p, proof)
                deferred.resolve()
            },
        })
}
