...
export async function testPointAdd(): Promise<boolean> {
    ...
    const { x: x1, y: y1 } = coordP1,
        ...
        res = await verifyPointAdd(params, P1X.p, P1Y.p, P2X.p, P2Y.p, P3X.p, P3Y.p, proof)
    if (!res) {
        return false
    }

    if (!serdeTest(PointAddProof, proof)) {
        return false
    }
